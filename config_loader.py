...
class ConfigLoader:
    ...
    def _read_config_from_file(self):
        ...
        try:
            ...
            logging.exception("Error decoding config file. Please check the file format.")
    ...
    def _validate_time_format(self, time_str):
        ...
        try:
            ...
            raise InvalidStructureError(f"Invalid time format: {time_str}. Expected format is HH:MM.")
    ...
    def _validate_config(self, config):
        ...
        try:
            ...
            self._validate_key_structure(config, required_keys, "Top level config error.")

            # Validate peak_hours
            if not all(isinstance(day, str) and isinstance(hours, list) for day, hours in config['peak_hours'].items()):
                raise InvalidStructureError("Invalid structure for peak_hours in config.")
            ...
            for day, time_slots in config['peak_hours'].items():
                ...
                for time_slot in time_slots:
                    if len(time_slot) != 2:
                        raise InvalidStructureError(f"Expected 2 time entries for each slot in {day}, got {len(time_slot)}.")
                    ...
                    if start_time_obj >= end_time_obj:
                        raise InvalidStructureError(f"Invalid time range in {day}. Start time {start_time} should be before end time {end_time}.")

                    # Check for overlaps with the previous time slot
                    if prev_end_time_obj and start_time_obj < prev_end_time_obj:
                        raise InvalidStructureError(f"Overlapping time range detected in {day} for {start_time} and {end_time}.")

                    prev_end_time_obj = end_time_obj  # update the previous end time


            # Validate fare_chart and cap_chart with a common pattern
            for key, validation_keys in {"fare_chart": {"peak", "non_peak"}, "cap_chart": {"daily", "weekly"}}.items():
            ...
            if fare_keys != cap_keys:
                ...
                errors = []
                if missing_in_fare:
                    errors.append(f"Combinations {', '.join(missing_in_fare)} found in cap_chart but missing in fare_chart.")
                if missing_in_cap:
                    errors.append(f"Combinations {', '.join(missing_in_cap)} found in fare_chart but missing in cap_chart.")
            ...
            logging.exception("Invalid line-to-line combination detected during config validation.")
            raise
        except Exception as e:  # This will catch any unexpected errors during validation
    ...
    def _validate_chart_structure(self, config_data, chart_key, validation_keys):
        ...
        for key, value in chart.items():
            if not set(value.keys()) == validation_keys:
                ...
                errors = []
                if missing_keys:
                    errors.append(f"Missing keys in {chart_key} for {key}: {', '.join(missing_keys)}")
                if extra_keys:
                    errors.append(f"Unexpected keys in {chart_key} for {key}: {', '.join(extra_keys)}")
                raise InvalidStructureError("; ".join(errors))
            
            if not all(isinstance(v, (int, float)) for v in value.values()):
                raise InvalidStructureError(f"Invalid values in {chart_key} for {key}. Values must be integers or floats.")