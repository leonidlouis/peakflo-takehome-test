...
def configure_log(log_level):
    ...
    caller_dir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
    ...
    if log_level.upper() == 'DEBUG':
        log_folder = os.path.join(caller_dir, "logs")  # Create the log folder in the caller's directory
        os.makedirs(log_folder, exist_ok=True)

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        file_handler = logging.FileHandler(f"{log_folder}/app_debug_{timestamp}.log")  # Save logs with a timestamp
...
def main():
    ...
    if not args.skip_tests:
        test_result = subprocess.run(['python3', '-m', 'unittest', 'discover','tests'], capture_output=True, text=True)

        if test_result.returncode != 0:
            logging.critical("Tests failed. Please fix the issues before running the main logic.")
            captured_output = test_result.stdout if test_result.stdout else test_result.stderr
